...
def test_orchestrator_raises_when_component_task_fails():
    async def run() -> None:
        ...
        with pytest.raises(RuntimeError, match="Background task 'watcher' failed") as exc_info:
            orchestrator._raise_for_failed_component_tasks()
...
def test_orchestrator_raises_when_component_task_exits_unexpectedly():
    async def run() -> None:
        ...
        with pytest.raises(RuntimeError, match="Background task 'watcher' exited unexpectedly"):
            orchestrator._raise_for_failed_component_tasks()
