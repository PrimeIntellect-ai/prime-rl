...
class CountingSampler(TaskSampler):
...
def test_default_curriculum_resumes_finite_and_infinite_tasksets() -> None:
    ...
    infinite = TaskSampler(task_stream())
    infinite.sample()
    infinite.sample()
    restored_infinite = TaskSampler(task_stream())
