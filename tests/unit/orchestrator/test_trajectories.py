...
class SimpleChatTokenizer:
    """Minimal tokenizer mock for testing message-based rollout reconstruction."""
...
def test_interleave_rollout_reconstructs_tokens_from_messages():
    ...
    rollouts = interleave_rollout(output, tokenizer=tokenizer)
    assert rollouts is not None
    assert len(rollouts) == 1
    ...
    assert rollout.completion_mask == (
        [True] * len(step1_completion_ids)
        + [False] * len(step2_new_prompt_ids)
        + [True] * len(step2_completion_ids)
    )
    assert rollout.completion_logprobs == [0.0] * len(rollout.completion_ids)


def test_interleave_rollout_missing_tokens_without_tokenizer_returns_none():
    output = vf.RolloutOutput(
        example_id=1,
        trajectory=[
            vf.TrajectoryStep(
