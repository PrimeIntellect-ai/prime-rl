...
def test_efficiency_tokens_with_tool_response_weight():
    ...
    result_completion_only = default_advantage_fn(inputs, length_penalty="tokens")
    ...
    result_tool_only = default_advantage_fn(
        inputs, length_penalty="tokens", completion_weight=0.0, tool_response_weight=1.0
    )
...
def test_efficiency_tokens_missing_tool_response_metric_is_zero():
    ...
    # Equivalent to default tokens shaping when no metric is present
    result_weighted = default_advantage_fn(
        inputs, length_penalty="tokens", completion_weight=1.0, tool_response_weight=1.0
    )
    result_default = default_advantage_fn(inputs, length_penalty="tokens")
    assert torch.allclose(result_weighted.advantages, result_default.advantages, atol=1e-6)
