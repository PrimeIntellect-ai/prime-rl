...
TOOL_CALL_PARSER_CASES: list[tuple[str, str]] = [
    # DeepSeek
...
REASONING_PARSER_CASES: list[tuple[str, str | None]] = [
    # GLM
    ("zai-org/GLM-4.5", "glm45"),
    ("zai-org/GLM-4.5-Air", "glm45"),
...
@pytest.mark.parametrize("model_name,expected_parser", TOOL_CALL_PARSER_CASES)
def test_auto_detect_tool_call_parser(model_name: str, expected_parser: str):
    assert resolve_tool_call_parser(model_name) == expected_parser


@pytest.mark.parametrize("model_name,expected_parser", REASONING_PARSER_CASES)
def test_auto_detect_reasoning_parser(model_name: str, expected_parser: str | None):
    assert resolve_reasoning_parser(model_name) == expected_parser
...
def test_unknown_model_returns_none():
    assert resolve_tool_call_parser("some/unknown-model") is None
