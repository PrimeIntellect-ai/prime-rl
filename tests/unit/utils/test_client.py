...
def test_setup_clients_assigns_renderer_and_dp_rank_headers():
    ...
    assert [client.client_type for client in clients] == [
        "openai_chat_completions_token",
        "openai_chat_completions_token",
    ]
    assert [client.renderer for client in clients] == ["qwen3_vl", "qwen3_vl"]
    assert [client.renderer_tokenizer_name_or_path for client in clients] == [None, None]
...
def test_setup_clients_assigns_renderer_model_name():
    ...
    assert clients[0].renderer_tokenizer_name_or_path == "Qwen/Qwen3-VL-4B-Instruct"
...
def test_setup_clients_preserves_chat_client_defaults():
    ...
    assert clients == [
        vf.ClientConfig(
            ...
            renderer="auto",
            renderer_tokenizer_name_or_path=None,
            api_key_var="PRIME_API_KEY",
