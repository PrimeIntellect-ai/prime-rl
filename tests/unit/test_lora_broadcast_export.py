...
def test_get_state_dict_for_run_exports_peft_standard_prefixes():
...
def test_get_state_dict_for_run_exports_peft_standard_prefixes_for_state_dict_for_adapter_modules():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            state = mgr.get_state_dict_for_run(0)
            assert 'base_model.model.model.layers.0.mlp.experts.0.gate_proj.lora_A.weight' in state
            assert 'base_model.model.model.layers.0.mlp.experts.0.gate_proj.lora_B.weight' in state
            assert 'base_model.model.model.layers.0.mlp.experts.1.up_proj.lora_A.weight' in state
        finally:
            dist.destroy_process_group()


def test_weight_checkpoint_adapter_export_does_not_double_prefix_run_keys():
    with tempfile.TemporaryDirectory() as td:
        init_file = Path(td) / 'pg_init'
        dist.init_process_group(backend='gloo', init_method=f'file://{init_file}', rank=0, world_size=1)
        try:
            ...
            ckpt_config = type('Cfg', (), {'save_format': 'safetensors', 'save_sharded': True, 'save_adapter_separately': True})()
