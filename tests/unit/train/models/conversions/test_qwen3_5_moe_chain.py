"""Mock equivalence: declarative chain == legacy imperative converter for qwen3_5_moe."""
...
from prime_rl.trainer.models.qwen3_5_moe.converting_qwen3_5_moe import convert_hf_to_tt_moe, convert_tt_to_hf_moe

from .conftest import assert_state_dicts_equal, clone

NUM_LAYERS, E, MOE, DIM, SHARED = 3, 4, 6, 8, 10
...
def _mock_hf(per_expert: bool = True) -> dict:
    ...
    for i in range(NUM_LAYERS):
        ...
        sd[f"{p}.mlp.shared_expert.gate_proj.weight"] = torch.randn(SHARED, DIM)
        sd[f"{p}.mlp.shared_expert.down_proj.weight"] = torch.randn(DIM, SHARED)
        sd[f"{p}.mlp.shared_expert.up_proj.weight"] = torch.randn(SHARED, DIM)
        sd[f"{p}.mlp.shared_expert_gate.weight"] = torch.randn(1, DIM)
    return sd


def test_qwen3_5_moe_forward_matches(per_expert=True):
    hf = _mock_hf(per_expert)
    chain = build_qwen3_5_moe_chain(NUM_LAYERS)
    imperative = clone(hf)
    convert_hf_to_tt_moe(imperative)
    declarative = apply_hf_to_tt(clone(hf), chain)
    assert_state_dicts_equal(imperative, declarative, "qwen3_5_moe hf->tt (per_expert)")


def test_qwen3_5_moe_forward_matches_fused():
    test_qwen3_5_moe_forward_matches(per_expert=False)


def test_qwen3_5_moe_backward_matches():
    ...
    assert_state_dicts_equal(imperative, declarative, "qwen3_5_moe tt->hf")
...
def test_qwen3_5_moe_roundtrip():
    ...
    assert_state_dicts_equal(hf, back, "qwen3_5_moe roundtrip")
