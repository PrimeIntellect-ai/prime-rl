...
def test_dense_mask_admits_the_final_kv_position():
    """A slot naming the last KV position must be admitted, and an `IGNORE_SLOT` (-1) slot must name nothing.

    The `IGNORE_SLOT` marker replaced a convention that appended a zero pad row to `kv_buf` and pointed
    ...
    indices = torch.tensor([[[[3, IGNORE_SLOT]], [[0, 1]]]], dtype=torch.int32)
    ...
    assert admitted[0].sum() == 1, "the `IGNORE_SLOT` slot admitted a position of its own"
