...
def test_short_rl_training(commit_hash: str, branch_name: str, username: str):
    ...
    training_cmd = f"uv run torchrun --nproc_per_node=1 src/zeroband/train.py @ configs/training/simple_reverse_two_gpu.toml --stop_after_steps 100 --wandb_run_name {wandb_run_name} --project {project}".split()
    inference_cmd = "uv run python src/zeroband/infer.py @ configs/inference/simple_reverse_two_gpus.toml --total_step 50".split()
