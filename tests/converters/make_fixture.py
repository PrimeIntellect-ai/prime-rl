"""Build a converter-test fixture for one mini_moe arch.

Creates a tiny random prime model, exports it as a source HF dir (the byte-exactness
reference), then loads it through the trainer's model setup and saves a DCP checkpoint
in run-dir layout. Building through the prime class keeps the source dir in the
canonical HF layout the converters emit, so comparisons need no canonicalization.

Usage: uv run python tests/converters/make_fixture.py <arch> <run_dir>
Writes <run_dir>/source, <run_dir>/checkpoints/step_1/trainer, <run_dir>/configs/trainer.json.
"""
...
def load_mini_moe():
    path = Path(__file__).parents[2] / "scripts" / "mini_moe.py"
