...
class EnvConfig(BaseConfig):
    ...
    def _migrate_and_resolve_env(cls, data):
        """Translate the flat env keys still emitted by hosted training's control plane
        into the composed ``env``/``serve``/``legacy`` blocks, then narrow ``env`` to the
        selected env's config class. One validator so migration always precedes narrowing."""
        data = migrate_legacy_env_config(data)
