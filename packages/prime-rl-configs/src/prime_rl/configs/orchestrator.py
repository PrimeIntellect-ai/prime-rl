...
class OrchestratorConfig(BaseConfig):
    ...
    """Maximum policy versions a rollout may train behind its generation policy, queue time included (batch N trains on policy v{N-1}, so a rollout generated by v{k} shipping in batch N is (N-1)-k versions off-policy). Rollouts already past this bound — in flight or buffered awaiting a batch — are discarded. Higher values yield better throughput at the cost of off-policy noise."""
