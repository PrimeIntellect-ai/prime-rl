...
class InferenceConfig(BaseConfig):
    tag: str = "student"
    """Identifier for this deployment within an RL run. Must be unique across all ``[[inference]]`` blocks. Used by the launcher to name per-deployment subconfigs, log files, and monitor entries, and by the orchestrator to route student/teacher requests by tag (only ``student`` and ``teacher`` are wired today). Launcher-only - not passed to vLLM."""
