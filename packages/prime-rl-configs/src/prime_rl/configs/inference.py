...
class InferenceConfig(BaseConfig):
    ...
    """Auto-set for sampling replay: return per-token sampling masks (``sampling_mask``) on ``/inference/v1/generate`` responses via vLLM's native ``--return-sampling-mask`` (>= 0.28). Capture is engine-wide: vLLM rejects requests with ``temperature <= 0`` or without ``top_k > 0`` while it is on, and it requires the V2 model runner (the launcher forces ``VLLM_USE_V2_MODEL_RUNNER=1``). Auto-enabled by the ``rl`` entrypoint under truncated train sampling and persisted into the per-node config; set by hand only for standalone-launched servers."""
