...
def propagate_shared_fields(data: Any) -> Any:
    ...
    def _student_entry() -> Any | None:
        """Return the dict for the ``tag = "student"`` entry of the tagged
        ``[[inference]]`` list (after ``_migrate_legacy_inference``), or None."""
        inference = data.get("inference")
        if not isinstance(inference, list):
            return None
        return next(
            (e for e in inference if isinstance(e, dict) and e.get("tag", "student") == "student"),
            None,
        )

    def get(path: str) -> Any | None:
        parts = path.split(".")
        # ``inference.*`` reads target the student entry of the tagged list.
        if parts[0] == "inference" and isinstance(data.get("inference"), list):
            node: Any = _student_entry()
            for p in parts[1:]:
    ...
    def fill(path: str, value: Any) -> None:
        parts = path.split(".")
        # ``inference.*`` writes route into the student entry of the tagged list.
        if parts[0] == "inference" and isinstance(data.get("inference"), list):
            student = _student_entry()
            if student is None:
                return
            node: Any = student
            for p in parts[1:-1]:
