...
| Checkpoint | `~/Desktop/codes/s3/experiments/${EXP_NAME}/checkpoints/` | `experiments/${EXP_NAME}/checkpoints/` | `/local-ssd/checkpoints/${EXP_NAME}` → rsync → S3 |
| 训练输出+渲染 | `~/Desktop/codes/s3/experiments/${EXP_NAME}/output/` | `experiments/${EXP_NAME}/output/` | `/local-ssd/prime-rl-output/` → rsync → S3（含 blendergym-work/） |
...
| Checkpoint (DCP) | ~18GB/step | `/local-ssd/checkpoints/` → rsync → S3 | S3 FUSE 不支持 rename，DCP 需要原子提交；本地写后定期 rsync |
| 渲染产物 (work_root) | 高频小文件 | `/local-ssd/prime-rl-output/blendergym-work` | 合并到 output_dir 下，rsync 自动覆盖；6 worker 并发写 |
| 训练输出 (output_dir) | 数 GB | `/local-ssd/prime-rl-output/` → rsync → S3 | rollout JSONL、wandb、渲染产物；排除 broadcasts/ 和 *.bin |
...
| `ckpt.output_dir` | (无) | `/local-ssd/checkpoints/${EXP_NAME}` | 现通过 CLI 覆盖 `--ckpt.output_dir /local-ssd/checkpoints/${EXP_NAME}`，TOML 值作为 fallback。本地 NVMe 写入 + 后台 rsync 到 S3（S3 FUSE 不支持 rename） |
