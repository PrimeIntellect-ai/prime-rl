...
uv run python -m prime_rl.entrypoints.env_server \
  @ examples/extra/dynamo/vlm/nemotron-3.5-super/env.toml
```
...
uv run torchrun \
  ...
  ' bash \
  @ examples/extra/dynamo/vlm/nemotron-3.5-super/trainer.toml \
  --output-dir "$RUN_DIR" \
...
uv run python -m prime_rl.entrypoints.orchestrator \
  @ examples/extra/dynamo/vlm/nemotron-3.5-super/orchestrator.toml \
  --output-dir "$RUN_DIR"
```
