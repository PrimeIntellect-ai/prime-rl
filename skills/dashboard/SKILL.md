...
The traces tab shows in-flight rollouts in the episode table (tinted, phase badge, counts that grow per turn, reward `n/a`; filter status → in flight / finished; click one to watch it grow in the viewer), folded from `monitors/file/traces/live/*.jsonl` at `/api/runs/<run>/live` and `/api/runs/<run>/live/<trace_id>`. The logs tab's components menu lists every env server under `all envs`, so one env's log can be toggled into its own pane. Replay defaults to 8× so long coding-agent model waits remain watchable; select
