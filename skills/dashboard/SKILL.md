...
For an eval run the metrics tab's toolbar is a filter button (env: all / any subset, colour-coded per env when several show; `include errors`) and a muted progress line; a block bar with one cell per expected episode (landed = env colour, errored = red, in flight = pulsing outline, click opens the episode or follows the live trace; percent beside the bar; the expected count comes from `monitors/file/eval_plan.json`, written when the epoch starts, else from the config), then sections of beeswarm panes (resizable chart cards) — scores (avg@k and pass@k over tasks), env metrics (`rewards/*`, `metrics/*`), usage (rollout time, in/out tokens, turns, branches, cost), and one zoomable timing pane (icicle of the zoomed node's mean composition + per-episode strips + breadcrumb + tree legend; click a segment/crumb/tree node to zoom, a strip to open the trace; `other` = parent minus children). Constants render as chips above a section, ≤8 distinct values as a counted dot plot, else a beeswarm. Phases use a ramp of the accent (envs keep the palette); strips group by env when several show. A dot is an episode or task: hover for its value, click to open the trace; hover the background for min/p10/p25/median/mean/p75/p90/max (a faint boxplot sits behind the dots). The filter button's `include errors` (default off) adds errored episodes, drawn red. The traces tab shows in-flight rollouts in the episode table (pulsing dot in `#`, phase badge, `dispatched → arrived (elapsed)`, counts that grow per turn, reward empty until the episode lands; errored episodes carry a red left border in place of a status column; status toggles in flight / completed; click one to follow it in the viewer, transcript only, pinned to the newest turn until you scroll up), polled once a second with an etag (`?etag=` answers `{unchanged}`), folded from `monitors/file/traces/live/*.jsonl` at `/api/runs/<run>/live` and `/api/runs/<run>/live/<trace_id>`. The logs tab's components menu lists every env server under `all envs`, so one env's log can be toggled into its own pane. Replay defaults to 8× so long coding-agent model waits remain watchable; select
