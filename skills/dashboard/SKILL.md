...
With `--monitors.prime` the top bar has a `view on platform` link (from `monitors/prime/run.json`: train id/url, or per-env evaluation urls; an eval's evaluation opens at epoch start and streams episodes as they land). For an eval run the metrics tab shows one env at a time; its toolbar is a filter button (env select, no all-envs option; `include errors`) and a muted progress line; sections are flat with muted headings (no collapse); a block bar with one cell per expected episode (landed = env colour, errored = red, in flight = pulsing outline, click opens the episode or follows the live trace; percent beside the bar; the expected count comes from `monitors/file/eval_plan.json`, written when the epoch starts, else from the config), then sections — summary tiles (avg@k, pass@k; error/truncation rate coloured >10% yellow, >50% red; mean turns / branches (one tile), mean episode time, mean tokens in+out, total cost if present; distribution on hover), `usage` (tokens: input | output) and `timing` (leaf phases boot/setup/model/harness/finalize/scoring + `other` = wall time minus phases) as composition panes: horizontal legend, icicle of the mean split, per-episode strips (click opens the trace; hover a part to dim the others; 40 strips by default, `show all` scrolls them all); env metrics (`rewards/*`, `metrics/*`) sit between summary and usage as beeswarm panes (resizable chart cards). Constants render as chips above a section, ≤8 distinct values as a counted dot plot, else a beeswarm (env metrics included). Phases use a ramp of the accent. A dot is an episode or task: hover for its value, click to open the trace; hover the background for min/p10/median/p90/max (a faint boxplot sits behind the dots). The filter button's `include errors` (default off) adds errored episodes, drawn red. The traces tab shows in-flight rollouts in the episode table (pulsing dot in `#`, phase badge, `dispatched → arrived (elapsed)`, counts that grow per turn, reward empty until the episode lands; errored episodes carry a red left border in place of a status column; status toggles in flight / completed; click one to follow it in the viewer, transcript only, pinned to the newest turn until you scroll up, the request in flight's uncommitted messages dimmed as `awaiting model`), polled once a second with an etag (`?etag=` answers `{unchanged}`), folded from `monitors/file/traces/live/*.jsonl` at `/api/runs/<run>/live` and `/api/runs/<run>/live/<trace_id>`. The logs tab's components menu lists every env server under `all envs`, so one env's log can be toggled into its own pane. Replay defaults to 8× so long coding-agent model waits remain watchable; select
