...
**Replay envs (continue / recheck)** — the built-in v1 `replay` taskset re-enters saved rollout records (`<output_dir>/rollouts/step_*/*_rollouts.jsonl`) as fresh tasks, scored by the original env's verifier: `taskset = { id = "replay", records = "<glob>", mode = "continue" | "recheck", source = { id = "<original taskset id>", ... } }`. `mode = "continue"` takes `anchor = "compaction"` (default, any harness — run with the source run's harness config) or `"tool-call"` (needs `default`/`null` harness, as does `recheck`). `max_seed_tokens` caps seeded context length. See "Replaying Past Rollouts" in `docs/configuration.md` and `deps/verifiers/verifiers/v1/tasksets/replay/`.
