...
**Replay envs** — `[orchestrator.train.env.taskset] id = "replay-v1"` replays saved rollouts (`<run>/rollouts/step_*/train_rollouts.jsonl`) as training tasks; a normal train env entry with a `ratio` (ratios are all-or-none across train envs). `mode = "continue" | "recheck" | "judge"` — one mode per env entry, judge on its own entry. `buffer_dir` is a prior run's rollouts dir, or `"self"` + `online = true` for this run's own growing buffer (forces whole-group dispatch). `inner` = the source run's taskset config — required for continue/recheck (they're scored by it), forbidden for judge (self-contained verdict-matching reward). Selection knobs: `max_candidates`, `max_steps_back`, `stop_conditions`, `source_envs` (allowlist over the `info.prime_rl.env_name` stamp; unset = every env except replay envs, listing a replay env opts into chained derivations), `allow_container`; judge knobs: `success_threshold`, `balance_labels`, `max_message_chars`, `max_transcript_chars`. Use the `null` harness and give the env entry explicit `max_input_tokens` / `max_total_tokens` headroom (replay seeds are long). Debug configs: `configs/debug/replay/`. See `docs/replay.md`.
