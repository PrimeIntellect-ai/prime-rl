...
**Replay envs (continue / recheck)** — the built-in v1 `replay` taskset re-enters saved rollout records (`<output_dir>/rollouts/step_*/*_rollouts_*.jsonl`) as fresh tasks, scored by the original env's verifier: `taskset = { id = "replay", records = "<glob>", mode = "continue" | "recheck", source = { id = "<original taskset id>", ... } }`. `mode = "continue"` takes `anchor = "compaction"` (default, any harness — run with the source run's harness config) or `"tool-call"` (needs `default`/`null` harness, as does `recheck`); for `"tool-call"`, `max_anchors` sets resume points per source rollout (default 1, `None` = every valid one). `max_seed_tokens` caps seeded context length. `records` globs are followed (append-only): pointing at the current run's own `rollouts/step_*/train_rollouts_<env>.jsonl` gives online self-replay. The orchestrator writes a derived `index_<env>.jsonl` beside each step's per-env record file; replay uses it to select records (task name / source reward / branch count) without parsing them — absent index = full parse. See "Replaying Past Rollouts" in `docs/configuration.md` and `deps/verifiers/verifiers/v1/tasksets/replay/`.
