...
**Replay envs** — `[orchestrator.train.env.taskset] id = "replay-v1"` replays saved rollouts (`<run>/rollouts/step_*/train_rollouts.jsonl`) as training tasks; a normal train env entry with a `ratio` (ratios are all-or-none, and replay envs need one — they advertise a single virtual task). Core fields: `buffer_dir` (a prior run's rollouts dir, or `"self"` for this run's own growing buffer — implies online + whole-group dispatch), `source_envs` (allowlist over the `info.prime_rl.env_name` stamp; unset = every env except replay envs, listing a replay env opts into chained derivations), `filters` (optional, e.g. `{ type = "stop-condition", allow = [...] }`). The task type is `[.taskset.derivation] type = "continue" | "recheck" | "judge"` — one per env entry; continue/recheck carry `inner` (the source run's taskset config, which scores them) and `allow_container`; recheck/judge carry `instruction`; judge carries `success_threshold` and `max_transcript_chars`. Continue/recheck run under the source env's harness (must support message prompts), judge under `null`; give the env entry explicit `max_input_tokens` / `max_total_tokens` headroom (replay seeds are long). Debug configs: `configs/debug/replay/`. See `docs/replay.md`.
