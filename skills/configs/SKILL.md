...
**RL loss** — `[trainer.loss]` defaults to IPO with `eps = 0.1`, `adv_tau = 1.0`, and `kl_tau = 1e-3`. Omit the section to use these defaults. Set `type = "custom"` with `import_path` and optional `kwargs` to load a custom RL loss. A custom loss that masks individual tokens should return `token_annotations={"is_masked": mask}` in `LossOutputs` so traces preserve its exact decision. The `ce` and `ref_kl` components are fixed.
