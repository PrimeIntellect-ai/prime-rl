...
CLI: `--orchestrator.train.envs.0.env.taskset.id reverse-text-v1`.
...
**Algorithms** — `[orchestrator.algo] type = "grpo" | "hierarchical_grpo" | "rae" | "max_rl" | "opd" | "opsd" | "sft" | "echo"` — the type names the algorithm (credit assignment + loss routing, fused), and each type's class defaults are its vetted setting; any other key you set is your own assembly (e.g. `[orchestrator.algo.roles.user] alpha = 0.1` for echo — setting any echo role replaces the whole role table). There is no preset layer, and no config hook that points at user code — a new algorithm is a named class in the repo (subclass `Algorithm`, register it). Per-env override: `[orchestrator.train.envs.algo] type = "opd"` (the env assembles its own algorithm). A multi-agent env (several trainable seats per episode) needs an algorithm that declares `multi_seat` — today `hierarchical_grpo` and `rae` (SPIRAL per-role EMA baselines, for zero-sum self-play); under a single-seat algorithm the first such episode fails the run (`UntrainableEnvError` — categorical, so it is not absorbed into error markers). A sole trainable seat beside frozen ones (agentic judge) is single-seat and trains under plain `grpo`. prime-rl only hosts the trainable policy; frozen models are inline external endpoints on the algorithm, named where the model is used — `[orchestrator.algo.teacher]` for opd (the frozen model scored against), `[orchestrator.algo.sampling.source]` for sft (the model it samples from), each with `name` + `base_url`. There is no shared `teacher` slot. opsd declares no model — it self-distills against the live policy. See `docs/algorithms.md`.
