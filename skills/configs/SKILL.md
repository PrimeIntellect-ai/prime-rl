...
**Hosted legacy shim (this branch)** — the `hrl` branch keeps accepting the pre-0.3.0 flat shapes hosted training's control plane emits: `[[orchestrator.train.env]]` / `[[orchestrator.eval.env]]` entries with flat `taskset` / `harness` (runtime nested inside) / `pool` / `num_workers` / `address` / `timeout` / `max_*_tokens` / v0 `id`+`args`, plus `max_inflight_rollouts`, `rollouts_per_example`, sampling `max_tokens`, the env server's old source-shaped `[env]` block, and the inference server's flat pre-vllm-block shape (`[model]`/`[parallel]` blocks, flat engine args like `--enable-lora`, and `--vllm-extra` JSON — all landing on `[inference.vllm]`). `packages/prime-rl-configs/src/prime_rl/configs/legacy.py` translates them onto the composed shape before validation. New configs must use the composed shape documented here — the shim exists only for the control plane.
