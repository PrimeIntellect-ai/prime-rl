...
For raw tool trajectories such as `PrimeIntellect/INTELLECT-5-SFT-Raw`, use `data.type = "sft_raw_tool_caterpillar"` or `data.type = "sft_raw_tool_caterpillar_per_branch"`. Use `data.type = "sft_raw_tool_caterpillar_grouped_branches"` when the baseline must preserve one full tree's branch set per micro-batch for mathematical equivalence checks. Use `data.type = "sft_raw_tool_current_rl_baseline"` when the baseline should match the current RL path: each assistant step trains on visible prompt context plus `reasoning_content` and visible assistant output, and steps only merge when the next prompt still extends the previous full sample. The loader treats assistant `reasoning_content` as reasoning side leaves while visible content/tool calls stay on the trunk. `seq_len` caps each root-to-leaf branch path; set `max_packed_tokens` separately when a FlexAttention tree run should admit larger DFS-packed trees. Use `selection_metric = "branching_score"` to prefer examples with many reasoning side branches instead of simply sorting by `num_turns`. Always set `max_examples` on baselines that expand examples into multiple samples to bound expansion.
Set `include_attn_mask = false` for raw-tool tree configs that use `model.attn = "flex_attention"`; otherwise the dataset will build a dense `[N, N]` SDPA mask that the FlexAttention path does not consume.
