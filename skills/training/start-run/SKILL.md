...
- NIXL on SLURM: install the UCX/NIXL build with `scripts/install_nixl_from_source.sh`, reinstall its wheel after `uv sync`, then install the ModelExpress and Redis binaries with `scripts/install_modelexpress.sh`. With the default local broadcast host, the generated job runs ModelExpress on the trainer head and injects its address into every component; a non-local `weight_broadcast.host` selects an external service. ModelExpress handles startup peer discovery; steady weight updates reuse prepared NIXL requests and versioned NIXL notifications. Each NIXL process pins UCX to its GPU's nearest active InfiniBand port unless `UCX_NET_DEVICES` is explicitly set, and inference ranks rotate their trainer-agent pull order across the available source rails. `weight_broadcast.overlap_transfer_and_replay = true` explicitly allocates a second trainer staging arena and inference receive arena so transfer and replay can overlap; leave it disabled when the extra largest-group-sized arena does not fit.
