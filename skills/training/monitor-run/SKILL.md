...
- **NIXL startup broadcast**: the trainer broadcasts policy v0 before waiting for rollouts. A message that the trainer is waiting for the receiver can be normal during inference warmup. Inference builds its transfer plan by tracing the vLLM weight loader on the first update; an `UnsupportedOpError` in `transports/weights/nixl/graph.py` is a graph-recording failure, before prepared RDMA reads. Inspect the source name and recorded operation chain. Terminal writes may arrive through either Python `Tensor.copy_` or native `aten.copy_` from indexed assignment; both use `LazyWeight._record_copy`. For HF export, pass the full model config so multimodal checkpoint prefixes survive; select conversion chains by `model_type`, since inference can use upstream config classes. `LazyWeight` records cat in the same operation chain as views and casts. Every terminal write uses the same graph lowerer, starting with one `RecordedCopy` and adding contributions at cat. It translates loader slices and destination layouts for each source; `apply_chain` evaluates tensor operations, and the existing single-source replay planner handles the resulting chains. Confirm `Applied NIXL policy update` on every inference rank and the orchestrator's completed policy update to establish transfer success.
