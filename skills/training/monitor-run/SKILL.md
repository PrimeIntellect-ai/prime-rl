...
The batches consumed by the trainer are shipped over the filesystem by default and land at `{output_dir}/rollouts/step_N/train_rollouts.bin`, next to the trace subtrees. With `rollout_transport.type = "zmq"` they ship in-memory instead and nothing binary is written.
