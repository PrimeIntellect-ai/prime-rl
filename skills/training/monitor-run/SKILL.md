...
- `{output_dir}/rollouts/step_N/{train,eval}/` — saved agent graphs (see Agent graphs below).
...
```
{output_dir}/rollouts/step_N/{train,eval}/all/traces.jsonl        # appended per graph as it completes
{output_dir}/rollouts/step_N/{train,eval}/effective/traces.jsonl  # written per finalized batch / eval epoch
```

JSONL files of `AgentGraph` records (training tensors excluded). `all` gets every completed
graph the moment it arrives — errored, filtered, and never-batched ones included — so it's
...
`kind`, `env_name`, `group_id`, `policy_version`, and `eval_step`; its `traces` retain agent,
parent, reward, metric, timing, and runtime metadata.

```bash
wc -l {output_dir}/rollouts/step_42/train/{all,effective}/traces.jsonl
jq '.traces[] | {agent, rewards}' {output_dir}/rollouts/step_42/train/effective/traces.jsonl
jq 'select(.error != null or any(.traces[]; .errors != [])) | {id, env_name, error}' {output_dir}/rollouts/step_*/train/all/traces.jsonl
```
