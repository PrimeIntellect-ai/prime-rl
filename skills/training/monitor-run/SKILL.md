...
| trainer | `sparse_update/{sparsity,changed_numel,patch_bytes,patched_tensors,diff_s,save_s}` | sparse filesystem weight-sync metrics when `trainer.weight_broadcast.type = "sparse_filesystem"` (`kernel_format = true` for GPU-direct application without CPU cache; `gpu_diff = true` to run the nonzero diff on GPU; `compress = true` for zstd-compressed patches; `diff_s`/`save_s` split the broadcast into diff vs disk-write time) |
