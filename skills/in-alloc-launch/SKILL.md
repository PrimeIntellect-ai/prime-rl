...
10. **Audit Slurm steps when node isolation matters.** After launching a lane in a shared allocation, immediately run `squeue --steps -j "$SLURM_JOB_ID"` and confirm that every non-batch step is on the requested lane hosts. If an agent/tool wrapper creates a long-running `bash` step on a different node, the run is tainted even if the generated lane script itself used the right `--deployment.hosts`. Cancel only the offending step(s), preserve logs, and relaunch from a shell whose long-running wrapper is allowed to occupy the lane.
11. **Stagger large inference fanout if vLLM startup livelocks.** Set `INFERENCE_START_STAGGER_SECONDS=30` before `rl` to delay inference node `N` by `N*30s`; default is 0. This preserves CUDA graph mode and avoids simultaneous 7-node vLLM startup pressure.
12. **Slingshot is libfabric (`cxi`), not InfiniBand.** Never use `ibv_devinfo` / `NCCL_IB_HCA`. The fabric loads via `module load brics/nccl`; without it NCCL silently falls back to TCP sockets (~10× slower). A healthy fabric does ≥~23 GB/s on `nccl-tests` and prints `NET/OFI` in `NCCL_DEBUG=INFO`.
13. **`/tmp` and `/dev/shm` are a shared, RAM-backed tmpfs that Slurm does NOT wipe between jobs.** A lane can hit `OSError [Errno 28] ENOSPC` from *other users'* leftover caches on a node — an environment issue, not a launcher bug. Pick fresh nodes or clear space; don't chase it as a code bug.
