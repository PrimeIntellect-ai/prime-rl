...
def _append_user_image_png(message: dict[str, Any], pil: Image.Image) -> None:
    """OpenAI-style image_url part; kept local so VeriHop works with older verifiers pins."""
    buf = BytesIO()
    ...
    c.append({"type": "image_url", "image_url": {"url": f"data:image/png;base64,{b64}"}})
...
def synthesize(
    ...
    for i in range(num_samples):
        ...
        _append_user_image_png(user_msg, img)
        prompt = [user_msg]
