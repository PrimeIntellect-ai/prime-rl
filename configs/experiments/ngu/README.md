...
Train `PrimeIntellect/GLM-4.5-Air-Scaleswe` on 800 SWE-rebench tasks, 200 per difficulty tier. The tiers rank 987 tasks solved by stored initial-policy rollouts; their mean pass rates are 99.7%, 73.2%, 42.6%, and 13.3%. Evaluate SWE-bench Verified and a disjoint 500-task SWE-rebench holdout every 20 steps.
...
uv run rl @ configs/experiments/ngu/train-static.toml @ configs/experiments/ngu/difficulty-eval.toml --slurm.project-dir "$PWD"
uv run rl @ configs/experiments/ngu/train-ngu.toml @ configs/experiments/ngu/difficulty-eval.toml --slurm.project-dir "$PWD"
```
