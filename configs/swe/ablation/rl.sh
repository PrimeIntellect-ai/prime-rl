...
# Paths
export BASE_DIR=${BASE_DIR:-"/shared/prime-rl-i4"}
...
if [ "$SLURM_PROCID" -lt "$NUM_INFER_NODES" ]; then
    ...
    uv run inference \
        @ configs/int3/ablation/infer.toml \
    ...
    if [ "$TRAIN_NODE_RANK" -eq 0 ]; then
        uv run orchestrator \
            @ configs/int3/ablation/orch.toml \
            --weight_broadcast.type nccl \
            ...
            --heartbeat.url $BETTER_STACK_URL \
    ...
    uv run torchrun \
        ...
        @ configs/int3/ablation/train.toml \
        --weight_broadcast.type nccl \
