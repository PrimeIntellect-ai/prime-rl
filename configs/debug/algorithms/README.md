...
Frozen models are plain `[orchestrator.models.<key>]` entries; the algorithm points at them by key (`algorithm = { name = "opd", model = "reverse-text-rl" }`). There is no dedicated teacher slot — the same entry can serve any number of envs' algorithms.
