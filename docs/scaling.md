...
`type = "overlapped_moe"` dispatches and combines through symmetric-memory CUDA kernels instead of an NCCL all-to-all, with its own always-bf16 fused expert FFN (`[trainer.model.moe.compute]` is ignored for this dispatch type). Both forward (dispatch+FFN) and backward (gradient send+FFN, including weight gradients) always run through a single CTA-specialized kernel each (producer CTAs scatter, consumer CTAs compute) — the only architecture in this package proven to achieve genuine intra-kernel compute/communication overlap, with no toggle or fallback. The forward kernel's GEMMs run on real Blackwell tensor cores via `tcgen05` (TMA-staged, ~5.5x faster than plain WMMA in isolation); the backward kernel's GEMMs are still plain WMMA. Experimental: only `score_before_experts = false` and no per-expert bias are supported (both fail loudly, not silently, if misconfigured), it needs `prime_kernels.fine_grained_compute_comm_overlap` built, and it's not yet a production speedup — full forward+backward is currently ~4.8x slower end-to-end than a plain `torch._grouped_mm` reference at real bench scale, down from ~5.9x before the forward kernel's tcgen05 conversion; backward's WMMA GEMMs are now the dominant remaining cost. See `OverlappedMoEDispatchConfig`'s and `overlapped_moe`'s package docstrings for the full scope, known gaps, and its `block_m`/`n_blocks`/`capacity_multiplier` tuning knobs.
