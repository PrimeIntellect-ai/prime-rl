...
You can run the trainer in isolation with fake data to debug without needing a real dataset or inference server.
...
Use variable-length fake data to more closely simulate real data:
...
Loss should drop from ~12 to ~2.5. The model won't be coherent, but it will have a non-trivial distribution so KL divergence is meaningful during RL.
...
What to look for:
- **Training runs without crashing** — validates the full pipeline
