...
- **Vision encoder frozen by default.** Set `freeze_vision_encoder = false` to fine-tune it; in that case it's FSDP-sharded per block. Has no effect under LoRA (LoRA freezes everything non-adapter regardless).
