...
6. **Context parallelism (optional).** CP-capable VLMs inherit `CPContextMixin` on every module which needs to be CP-aware and shard embeds/positions inside the model after the vision merge; the trainers defer sharding to the model for MRoPE batches under ulysses.
