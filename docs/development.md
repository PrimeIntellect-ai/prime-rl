...
4. **Packed-boundary consumption.** Samples pack into shared rows with per-document boundaries in `seq_lens`; the model consumes them via its `prime_forward_kwargs` override (varlen flash `cu_seqlens`, linear-attention state resets per document). Set `self.supports_packed_multimodal_training` on VLM configs once packed rows are handled — RL fails loudly at startup for VLM models without it.
