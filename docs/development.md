...
6. **Context parallelism (optional).** CP-capable VLMs implement `setup_context_parallel` on all modules which need to be CP-aware and shard embeds/positions inside the model after the vision merge; the trainers defer sharding to the model for MRoPE batches under ulysses.
