...
Trainer checkpoints are DCP-sharded; export them to HF-format safetensors with `tools/converters/dcp_to_bf16.py`. The script reads the model config from the run's resolved config and writes sharded safetensors plus config/tokenizer assets to `<ckpt_dir>/weights` (or a second positional arg). It exports full fine-tunes only — LoRA checkpoints are rejected.

```bash
# single process (1 GPU)
uv run python tools/converters/dcp_to_bf16.py outputs/my-run/checkpoints/step_10
