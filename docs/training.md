...
| `orchestrator.max_off_policy_steps` | Maximum staleness (in policy versions) a token may have and still contribute to the loss (default 8, `None` disables). Rollouts are never dropped for going off-policy — generation continues across in-flight weight updates, and tokens sampled more than this many versions behind the shipping policy are masked out of the loss (kept as context). The main off-policy dial on long agentic rollouts — raise for more trainable tokens, lower for tighter on-policyness. Watch `off_policy/masked_token_rate` and `mismatch_kl/all/mean` when tuning. |
