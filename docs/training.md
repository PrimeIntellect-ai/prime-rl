...
| `uv run env-server` | Standalone env server for one environment. | Takes the same `[env]`/`[serve]`/`[legacy]` blocks as verifiers' `serve` CLI. The `rl` launcher starts these automatically (one per train/eval source, at the source's derived `serve.address`); only needed when running the orchestrator standalone. |
