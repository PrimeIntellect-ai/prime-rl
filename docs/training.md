...
| `--run.project <name>` | Project the run belongs to (default `prime-rl`). A project groups runs the way a W&B project does — training and eval runs alike — and is a tag on the run, not a directory: the dashboard lists projects and lets you pick and overlay the runs of one, and the W&B project inherits it unless set explicitly. |
...
W&B is off by default (the file monitor, which writes the metrics and the trace stream under the run's `monitors/file/` directory, is on by default). Both file the run under `run.project` unless their own `project` is set:

```bash
uv run rl @ rl.toml --monitors.wandb                      # project run.project, name run.name
uv run rl @ rl.toml --run.project my-proj --run.name run-42
uv run rl @ rl.toml --monitors.wandb.project other-proj   # a W&B project of its own
