...
| `[[orchestrator.train.envs]]` | Training environments. List multiple tables for multi-env training; weight them via `ratio`. See [Configuration § Environments](configuration.md#environments-orchestratortrainenvs). |
| `[[orchestrator.eval.envs]]` + `orchestrator.eval.interval` | Eval environments and cadence (default every 100 steps). |
