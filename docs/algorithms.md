...
| `dataset_sft` | *(none)* | `ce` on dataset tokens | SFT from a static HF dataset through the RL stack: the orchestrator renders dataset examples (`messages` or `prompt`/`completion` columns, like the native SFT trainer) into ce-routed samples and ships them directly — no envs, no rollout sampling. Run-level only: train sources must be empty; eval sources still run online evals. Needs `dataset` (`[orchestrator.algo.dataset] name = "..."`). |
