...
Step indices are 1-indexed; policy versions are 0-indexed, with $\pi_0$ the base model. At step 1 inference samples from $\pi_0$, so the gap holds at startup — inference is exactly one step behind the trainer.
