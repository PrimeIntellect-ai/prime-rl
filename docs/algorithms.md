...
| `advantage.type` | Class | `assign_advantages` (group time) | `query_references` (ship time) |
|---|---|---|---|
...
`OPDAlgorithm` / `OPSDAlgorithm` have an async ship-time half (`query_references`): at batch-ship time they query their teacher (`model`, a [model reference](#model-references)) with bounded concurrency (`max_concurrent`, default 32) and attach per-token reference logprobs to each sample:
