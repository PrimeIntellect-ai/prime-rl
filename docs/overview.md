...
The three processes communicate through configurable transports — by default the trainer↔orchestrator rollout link uses the shared filesystem (rollouts persisted under the run's output dir, so trainer and orchestrator only need a common volume), and weight broadcast uses NCCL for synchronous in-memory transfer (falling back to filesystem when LoRA is enabled or no inference server is configured). Swap the rollout link to ZMQ (`rollout_transport.type = "zmq"`) for in-memory shipping when both processes share a host. See [Scaling](scaling.md) for the deployment options.
