...
The three processes communicate through configurable transports — by default the trainer↔orchestrator rollout link goes through the filesystem, and weight broadcast uses NCCL for synchronous in-memory transfer (falling back to filesystem when LoRA is enabled or no inference server is configured). Swap the rollout link to ZMQ (`rollout_transport.type = "zmq"`) to keep batches in memory; the trainer and orchestrator must then either be co-located or point at each other with `rollout_transport.host`. See [Scaling](scaling.md) for the deployment options.
