...
- **`G` — expected episode cost in tokens.** Per env, step-clocked EWMAs of final context size `G_e` and duration `d_e` — completions accumulate between step boundaries and fold in as one interval mean per step, so heavy-tailed episode lengths average out before they can move the estimate (bootstrap: the engine's `max_model_len`) — aggregated as `G = Σ w_e G_e / Σ w_e`. Train envs weigh in by sampling `ratio × d_e`; while an eval epoch is in flight, the eval mix (weighted by remaining scheduled episodes `× d_e`) interpolates with the train mix on the eval share of the cap — `min(1, remaining / n_max)` — so the price jumps toward the eval profile when an epoch fires and decays continuously back to the train profile as it drains. The `d_e` factor corrects length bias: long-episode envs are overrepresented in flight. Cost never influences *which* episode is admitted — scheduling stays FIFO and env-unbiased; the estimates only size the shared permit pool.
