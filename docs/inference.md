...
- **`G` — expected episode cost in tokens.** Per env, step-clocked EWMAs of final context size `G_e` and duration `d_e` — completions accumulate between step boundaries and fold in as one interval mean per step, so heavy-tailed episode lengths average out before they can move the estimate (bootstrap: the engine's `max_model_len`) — aggregated as `G = Σ w_e G_e / Σ w_e`. Train envs weigh in by sampling `ratio × d_e`; eval envs, while an eval epoch is in flight, by scheduled episodes (`num_examples × group_size`) `× d_e`. The `d_e` factor corrects length bias: long-episode envs are overrepresented in flight. Cost never influences *which* episode is admitted — scheduling stays FIFO and env-unbiased; the estimates only size the shared permit pool.
