...
The orchestrator continuously sizes how many episodes run against the inference engines at once. A static cap cannot be right: too low starves the engines, while too high adds queues and can cross into KV thrash. The optimal value depends on the model, deployment, and workload, and it moves as training changes the policy.

The controller treats the deployed engines as ground truth instead of classifying the model or predicting rollout cost. After a completed pipeline turnover, it measures generation throughput at the current cap, probes a higher cap, then returns to the incumbent for a confirmation window. Upward probes are accepted while they remain within five percent of incumbent throughput, favoring the largest useful async work reservoir. Throughput never lowers the episode cap: agentic episodes can be inside sandboxes, verifiers, or tool calls, so low active inference demand is not evidence of excessive episode concurrency. This bracketed comparison rejects slow workload drift and applies to compute-bound linear attention, KV-bound full attention, and hybrid models.

- **Probe up**: after the in-flight pool has turned over, and while its cap binds admission and the engines report fresh, queue-free throughput, test multiplicatively higher concurrency. Failed probes return softly to the incumbent and never cancel rollout work. Ties prefer higher concurrency, up to the user's maximum, so async rollouts can hide environment and long-tail latency. At the configured ceiling there is nothing to probe, so the controller holds unless a pressure signal requires a reduction.
...
- **Cut**: when the engines are demonstrably overloaded — requests being preempted, or a substantial capacity queue persisting across several polls — cut the pool hard, cancel the excess, and hold further cuts until the system settles.
