...
The orchestrator sizes its in-flight episode cap adaptively. A static cap is workload-dependent: too low starves the engines; too high triggers KV thrash (preemption → re-prefill churn → prefix-cache eviction → pile-up), and recovery is slow.
...
- **`G` — expected episode cost in tokens**, the max of two estimators. (1) *Completion mix*: per env, bias-corrected per-episode EWMAs of final context size `G_e` and duration `d_e` with a ~512-episode window, aggregated with duration-corrected weights — train envs by sampling ratio, eval envs (while an epoch is in flight) interpolating on the eval share of the cap so the price glides between the train and eval profiles. (2) *Measured request cost*: a size-biased mean (`E[X²]/E[X]`) of prompt tokens per request off the vLLM histogram bucket deltas each poll, plus the mean generation tokens. Every turn resends its episode's full context, so the request stream is a live census of in-flight episode sizes — it cannot be dragged down by fast finishers the way the completion mix can (after a cap raise, short episodes complete first), and size-biasing weighs each request by its own token mass, matching how it occupies KV (a plain per-request mean under-prices ~2×: small contexts take turns far more often than the large residents that fill the cache). Envs without completions fall back to the measured cost, and to the engine's `max_model_len` only before any traffic exists. Cost never influences *which* episode is admitted — scheduling stays FIFO and env-unbiased; the estimates only size the shared permit pool.
- **`κ` — over-commit factor**, starts at 1. Absorbs what the cost model cannot see: generate-call duty cycle, prefix sharing, offload headroom, and the gap between an episode's current and final size. Driven by a per-engine overload signal, worst engine wins: **HARD** = preemptions in the poll window (`Δ vllm:num_preemptions_total > 0`) *or* capacity-queued requests above `0.5 ×` running for 12 consecutive polls (agentic rollouts overload by queueing, not preempting — admission control parks excess load in the waiting queue), **SOFT** = waiting queue non-empty at two consecutive polls, **CLEAR** otherwise.
    - HARD: cut once — preemption cuts to `0.75 × inflight`, queue cuts to `0.9 × running` (what the engines demonstrably serve); a cut never raises the cap. Then freeze cuts until inflight drains below the new cap; repeated cuts on the stale draining signal would ratchet to the floor. If HARD survives a full drain, cut `×0.5`. Every cut also pins a **κ ceiling** at `0.9 ×` the κ that overloaded, so the ceiling is rediscovered by a wobble instead of a fresh thrash episode every cycle; the ceiling relieves slowly (~+2% per 10 min) so a genuinely lightened workload can re-probe.
    - SOFT: hold — saturated-but-not-thrashing is the operating point.
    - CLEAR: grow `κ ×1.003` **per poll** (clocked on polls, not steps: step duration varies from seconds to tens of minutes across workloads), only while the cap binds (`inflight ≥ 0.9 × n_max`) and no request is capacity-queued — thrash onset is a cliff, and capacity-queuing means the KV blocks are full right now.

`n_max` is re-evaluated **every poll** from the current `G` and `κ`, with a ±2% deadband so admission does not churn on estimate noise. The controller is fully step-independent — its clock is the **pipeline turnover**: each completed episode advances the turnover by `1 / inflight`, so one turnover means the in-flight pool has been replaced once, regardless of concurrency scale or step cadence. Raises are slew-limited to `×1.25` per turnover — 100 completions at concurrency 10 buy 10× the raise budget of 100 completions at concurrency 1000, matching how much less biased their estimates are; this is also what makes the cold start safe (the cap cannot outrun the pipeline's demonstrated stability). Cuts and decreases are never limited.

Refills are burst-capped (at most `max(group_size, 0.1 × n_max)` new admissions per poll interval) so recovery never lands a cap's worth of prefills at once. The cap starts at `initial_inflight` (or, when unset, a pessimistic runtime bound: KV capacity / the engine's `max_model_len`, read from `/v1/models`). `κ` initializes on the first poll with real cost data (measured traffic or a completion) as `κ ← max(1, n_max · G / C)`: a user-set start that implies over-commit is respected (continuity), otherwise the cap ramps from the safe full budget (`κ = 1` means the summed cost estimates of everything in flight fit in `C` even at peak size).
