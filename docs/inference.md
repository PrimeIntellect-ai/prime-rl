...
The cap starts from a safe bound derived from the engines (KV capacity divided by the maximum context length), or from `initial_inflight` when a good value is known, and always stays within `[min_inflight, max_inflight]`. The cap bounds the pool, not the rate at which it fills; `tasks_per_minute` adds an admission rate limit in episodes per minute, which paces sandbox creation for sandbox-backed environments.
