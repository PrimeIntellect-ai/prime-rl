...
The controller treats the deployed engines as ground truth instead of classifying the model or predicting rollout cost. It measures generation throughput at the current cap, probes a neighboring cap, then returns to the incumbent for a confirmation window. Upward probes are accepted while they remain within five percent of incumbent throughput, favoring the largest useful async work reservoir. Downward probes are accepted only when they materially improve throughput. This bracketed comparison rejects slow workload drift and automatically follows compute-bound linear attention, KV-bound full attention, and hybrid models.

- **Probe**: while the cap binds admission and the engines report fresh, queue-free throughput, test multiplicatively higher or lower concurrency. Failed probes always return softly to the incumbent; they never cancel rollout work. Ties prefer higher concurrency, up to the user's maximum, so async rollouts can hide environment and long-tail latency. Periodic downward probes detect when a changed workload has pushed the incumbent beyond the efficient throughput plateau.
