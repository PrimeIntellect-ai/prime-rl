...
- **`model.cp` + AFMoE**: Context parallelism now works with AFMoE models. Added `substitute_afmoe_flash_attn` which patches `AfmoeFlashAttention` to use ring attention with per-layer `window_size` (sliding window for local layers, full attention for global layers). Also plumbed `window_size` through the FA3 ring attention wrapper (`ring_fa3_varlen_func`). Backward-compatible: all new parameters default to full attention `(-1, -1)`, existing models and configs are unaffected. (2026-02-21)
