...
- **`orchestrator.token_batch_size`** and **`orchestrator.max_inflight_rollouts`**: Replaced throughput controls previously configured via `batch_size` and `max_async_level` in RL configs. Use `token_batch_size` to control trainer accumulation and `max_inflight_rollouts` to control orchestrator concurrency (2026-02-17)
