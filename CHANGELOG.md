...
- **`log.json_logging`**: Added JSON structured logging option for log aggregation systems (Loki, Grafana, etc.). When enabled, uses loguru's `serialize=True` for newline-delimited JSON output. Available on root `log`, `trainer.log`, and `orchestrator.log` (default: False) (2026-01-28)
