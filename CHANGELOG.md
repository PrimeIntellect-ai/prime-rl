...
- **`orchestrator.batch_size`**, **`orchestrator.token_batch_size`**, **`orchestrator.max_inflight_rollouts`**, **`orchestrator.oversampling_factor`**: Added token-based batching via `token_batch_size`; added explicit in-flight control via `max_inflight_rollouts`; `batch_size` is now optional and mutually exclusive with `token_batch_size` (if neither is set, batching defaults to rollout mode with `batch_size=128`); and removed `oversampling_factor` (2026-02-23)
