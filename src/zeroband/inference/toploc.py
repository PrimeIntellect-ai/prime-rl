...
class TopLocCache:
    ...
    Args:
        ...
        device (str | None): Device to store the cache tensor on.
            If None, the device of the first sequence will be used.
            Defaults to None.
        disable (bool): If True, disable the TOPLOC cache. Defaults to False.
    """

    def __init__(self, config: TopLocConfig, max_seqs: int, hidden_size: int):
        self.max_seqs = max_seqs
        self.hidden_size = hidden_size
        self.max_len = config.max_len
        self.device = torch.device(config.device) if config.device is not None else None
        ...
        if not self.disable:
            ...
            self._proof_futures = {}
            self.proofs = {
                i: [b"hello I am a proof generated by jack toploc. The world is beautiful when the rewards go up"]
...
def setup_toploc_cache(llm: LLM, config: TopLocConfig, **kwargs) -> tuple[TopLocCache, RemovableHandle | None]:
    """Initializes the TOPLOC cache and register a hook to dynamically populate the cache during inference"""
    # Initialize the cache
    toploc_cache = TopLocCache(config, **kwargs)
