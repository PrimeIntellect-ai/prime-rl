# Transformers v5 removed several attributes from PretrainedConfig that vLLM 0.16.0 still expects.
# Because vLLM uses spawn for worker processes, monkey-patches in the parent don't propagate.
# We install a .pth hook into site-packages so the patch runs at Python startup in every process.
_COMPAT_MODULE = "prime_rl_transformers_compat"

_COMPAT_SOURCE = '''\
"""Transformers v5 compatibility patches for vLLM (auto-generated by prime-rl)."""
...
def install_transformers_compat_hook():
    """Install a .pth startup hook so spawned vLLM workers get transformers v5 compat patches."""
    import site

    site_dir = site.getsitepackages()[0]
    py_path = f"{site_dir}/{_COMPAT_MODULE}.py"
    pth_path = f"{site_dir}/{_COMPAT_MODULE}.pth"

    with open(py_path, "w") as f:
        f.write(_COMPAT_SOURCE)
    with open(pth_path, "w") as f:
        f.write(f"import {_COMPAT_MODULE}\n")
