...
def _global_inference_rank(
    *,
    ...
    if data_parallel_size != 1 and data_parallel_size != logical_data_parallel_size:
        raise ValueError(
            f"data parallel size {data_parallel_size} does not match engine-derived size {logical_data_parallel_size}"
        )
    ...
    if not rank_offset <= rank < rank_offset + engine_world_size:
        raise ValueError(
...
class NIXLWeightUpdateWorker(Worker):
    ...
    def init_broadcaster(
        ...
        if engine_world_size is None:
            ...
            global_rank = _global_inference_rank(
