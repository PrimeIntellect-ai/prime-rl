...
def load_weights_checkpoint_layerwise(
    ...
    """Load checkpoint-format weights streamed from the trainer.

    This is vLLM's own weight-loading path under the layerwise reload
    lifecycle: ``model.load_weights`` maps the checkpoint names onto the
    engine's (TP/EP-sharded, possibly fused or quantized) parameters, each
    parameter's ``weight_loader`` performs the slicing and fp8 layout
    handling, and the finalize step re-runs quantization and attention
    post-processing (including the MLA absorbed weights). The engine
    therefore accepts exactly the tensor set and naming a matching
    checkpoint on disk would have — no engine-side remapping of the wire
    stream is needed, for bf16 and blockwise-fp8 transfers alike.
    """
