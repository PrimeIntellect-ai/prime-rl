...
class AfmoeForCausalLM(nn.Module, SupportsPP, SupportsLoRA):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.model = AfmoeModel(vllm_config=vllm_config, prefix=maybe_prefix(prefix, "model"))
        if get_pp_group().is_last_rank:
    ...
    def compute_logits(self, hidden_states: torch.Tensor, sampling_metadata=None) -> Optional[torch.Tensor]:
