...
@jaxtyped(typechecker=typechecker)
def forward_with_multilora(
    model: nn.Module, input_ids: Int[Tensor, "batch seq"], position_ids: Int[Tensor, "batch seq"]
) -> Float[Tensor, "batch seq vocab"]:
    hidden_states: torch.Tensor = model.model.embed_tokens(input_ids)

    if model.model.config._attn_implementation in ("flash_attention_2", "flash_attention_3"):
    ...
    position_embeddings: torch.Tensor = model.model.rotary_emb(hidden_states, position_ids)
    ...
    logits: torch.Tensor = model.lm_head(hidden_states)
