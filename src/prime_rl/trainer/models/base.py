...
class PreTrainedModelPrimeRL(PreTrainedModel):
    ...
    def convert_layer_to_vllm_kernel(
        ...
        This produces weights in the format vLLM uses at inference time (fused projections,
        stacked experts, optional FP8 quantization), enabling direct in-place weight updates
        via NCCL broadcast without going through the HF checkpoint format.

        Args:
            state_dict: The state dict containing the layer's weights in PrimeRL format.
            layer_idx: The index of the layer to convert.
            quantize_fp8: Whether to quantize weights to FP8 (e4m3) with block-wise scaling.

        Returns:
            A new dict with weights in vLLM kernel format.
        """
