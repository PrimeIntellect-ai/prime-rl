...
class _QNormRoPE(torch.autograd.Function):
    ...
    @staticmethod
    def forward(
        ctx, q: torch.Tensor, cos_sin_cache: torch.Tensor, position_ids: torch.Tensor, eps: float
    ) -> torch.Tensor:
        rows = q.reshape(-1, q.shape[-1])
        normed, rstd = _rmsnorm_fwd(rows, eps)
        normed = normed.view(q.shape)
        mla_rope_apply_raw_(normed, cos_sin_cache, position_ids, False)
        ctx.save_for_backward(rows, cos_sin_cache, position_ids, rstd)
        return normed.to(q.dtype)

    @staticmethod
    def backward(ctx, grad: torch.Tensor):
        rows, cos_sin_cache, position_ids, rstd = ctx.saved_tensors
        # Rotates `grad` in place: unsafe if autograd hands this same tensor to another consumer.
        grad = grad.contiguous()
        mla_rope_unapply_raw(grad, cos_sin_cache, position_ids, False)
        return _rmsnorm_bwd(rows, grad.view(rows.shape), rstd).view(grad.shape), None, None, None


def q_norm_rope(q: torch.Tensor, cos_sin_cache: torch.Tensor, position_ids: torch.Tensor, eps: float) -> torch.Tensor:
    """Unweighted RMSNorm over `head_dim`, then interleaved RoPE, both in fp32, rounded once to `q`'s dtype.

    `q` is `(t, h, d)` or `(1, t, h, d)` and contiguous; `cos_sin_cache` and `position_ids` are as in
    `apply_interleaved_rope_`.
    """
    return _QNormRoPE.apply(q, cos_sin_cache, position_ids, eps)
