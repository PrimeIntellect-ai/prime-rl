...
from prime_rl.trainer.models.hy_v3.configuration_hy_v3 import HYV3Config
from prime_rl.trainer.models.hy_v3.converting_hy_v3 import (
...
@auto_docstring
class HYV3PreTrainedModel(PreTrainedModelPrimeRL):
    config: HYV3Config
    ...
    def is_hf_state_dict(cls, state_dict: dict[str, Tensor]) -> bool:
        ...
        # Per-expert format (hub checkpoint) or fused format (transformers 5.0+)
...
class HYV3Model(HYV3PreTrainedModel):
    # MTP layer (index num_hidden_layers, speculative decoding only) is not trained
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        routed_experts (`torch.LongTensor` of shape `(batch_size, sequence_length, num_hidden_layers, num_experts_per_tok)`, *optional*):
            Routed experts for each token in the sequence. Only used for router replay.
        """
        assert use_cache is None, "use_cache is not supported for custom hy_v3 for now"
        assert past_key_values is None, "past_key_values is not supported for custom hy_v3 for now"
...
__all__ = ["HYV3Config", "HYV3PreTrainedModel", "HYV3Model", "HYV3ForCausalLM"]
