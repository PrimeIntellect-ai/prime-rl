"""Declarative HF<->prime conversion chain for Laguna.

Mirrors :mod:`prime_rl.trainer.models.conversion_chains`; equivalence to the
legacy imperative converter
(:mod:`prime_rl.trainer.models.laguna.converting_laguna`) is checked in
``tests/unit/train/models/conversions/test_laguna_chain.py``.
...
from prime_rl.trainer.models.conversion_chains import _GATE_DOWN_UP, _routed_experts_op
...
def build_laguna_chain(num_layers: int) -> list[ConvOp]:
    ops: list[ConvOp] = []
