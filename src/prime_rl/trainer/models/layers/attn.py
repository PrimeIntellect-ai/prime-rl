...
class FlexAttentionKernel(torch.nn.Module):
    ...
    _compiled_flex_attn = torch.compile(
        flex_attention, mode="max-autotune-no-cudagraphs" if "DEBUG" not in os.environ else None
    )
...
class FlexAttention(nn.Module):
    """
    FlashAttention-like module, but backed by PyTorch FlexAttention
    instead of flash_attn_varlen_func.

    Same public forward signature / behavior pattern as your FlashAttention:
      - q/k/v projections
      - optional q,k RMSNorm
      - RoPE
      - grouped-query attention expansion
      - varlen packed attention (causal per sequence)
      - o_proj
    """
    ...
    def forward(
        ...
        out = self._flex_attention_kernel(
            query_states,
            key_states,
            value_states,
            block_mask=mask,
