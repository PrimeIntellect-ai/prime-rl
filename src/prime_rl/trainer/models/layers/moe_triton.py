...
@triton.jit
def _histogram_kernel(index_ptr, out_ptr, n_elements, expert_num, BLOCK_N: tl.constexpr):
    pid = tl.program_id(0)
...
def _moe_scatter_kernel(
    ...
    idx = tl.load(index_ptr + pid_n).to(tl.int32)
    ...
    in_ptrs = in_ptr + pid_n * stride_in0 + offs_h * stride_in1
...
def _moe_weighted_gather_kernel(
    ...
    for k in range(0, top_k):
        ...
        w = tl.load(weight_ptr + pid_t * stride_w0 + k * stride_w1).to(acc_dtype)
...
def triton_histogram(top_k_rank: torch.Tensor, expert_num: int) -> torch.Tensor:
    ...
    _histogram_kernel[(triton.cdiv(flat_i32.numel(), 256),)](flat_i32, counts, flat_i32.numel(), expert_num, BLOCK_N=256)
    return counts
...
class _TritonMoEScatter(torch.autograd.Function):
    ...
    def forward(ctx, input: torch.Tensor, index: torch.Tensor, out_size: int | None = None) -> torch.Tensor:
        ...
        hidden_dim = input.shape[-1]
        out_size = int((index >= 0).sum().item()) if out_size is None else int(out_size)
        out = input.new_zeros((out_size, hidden_dim))
        ...
        block_h = 128
        _moe_scatter_kernel[(flat_index.numel(), triton.cdiv(hidden_dim, block_h))](
            flat_input,
            flat_index,
            out,
            flat_index.numel(),
...
def _compute_grad_weight_chunked(input: torch.Tensor, index: torch.Tensor, grad_out: torch.Tensor) -> torch.Tensor:
    ...
    bytes_per_topk = token_num * hidden_dim * max(input.element_size(), torch.empty((), dtype=compute_dtype).element_size())
    ...
    for start in range(0, top_k, chunk_k):
        ...
        gathered = input.index_select(0, safe_index[:, start:end].reshape(-1)).reshape(token_num, end - start, hidden_dim)
