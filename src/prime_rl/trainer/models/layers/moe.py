...
class MoE(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # When on: the shared expert overlaps with the routed dispatch/combine a2a on a side CUDA stream.
        self._overlap_shared_expert = False

    def set_token_dispatcher(self, token_dispatcher: TokenDispatcher) -> None:
        self.token_dispatcher = token_dispatcher

    def set_overlap_shared_expert(self, enabled: bool) -> None:
        self._overlap_shared_expert = enabled
    ...
    def _dispatch_experts_overlapped(
        self,
        x: torch.Tensor,
        top_scores: torch.Tensor,
        selected_experts_indices: torch.Tensor,
        ...
        with torch.cuda.stream(comm_stream):
            ...
            dispatch_done = comm_stream.record_event()
        shared_output = self.shared_expert(x)
        ...
        comm_stream.wait_stream(master_stream)
        with torch.cuda.stream(comm_stream):
            routed_output.record_stream(comm_stream)
            routed_output = self.token_dispatcher.combine(routed_output, state)
            barrier = comm_stream.record_event()
        master_stream.wait_event(barrier)
        routed_output.record_stream(master_stream)
        self.token_dispatcher.synchronize()
