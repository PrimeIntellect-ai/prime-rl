...
# Which low-precision recipe to use for the MoE expert grouped GEMM. ``None`` keeps bf16,
# ``"fp8"`` uses DeepGEMM blockwise FP8 (SM90), ``"mxfp8"`` uses torchao MXFP8 (SM100).
GroupedMMQuant = Literal["fp8", "mxfp8"] | None
...
class MoEArgs:
    ...
    grouped_mm_quant: GroupedMMQuant = None  # low-precision recipe for the expert grouped GEMM
...
def _run_experts_fp8_grouped_mm(
    ...
    return _run_experts_grouped_mm_impl(w1, w2, w3, x, num_tokens_per_expert, grouped_mm_quant="fp8")
...
def _run_experts_mxfp8_grouped_mm(
    ...
    return _run_experts_grouped_mm_impl(w1, w2, w3, x, num_tokens_per_expert, grouped_mm_quant="mxfp8")
...
class MoE(nn.Module):
    def __init__(self, moe_args: MoEArgs, dim: int, hidden_dim: int):
        ...
        self.experts = GroupedExperts(
            ...
            grouped_mm_quant=moe_args.grouped_mm_quant,
        )
        self.ep_comm_backend: EPCommBackend = "torch"
...
def _run_nongated_experts_fp8_grouped_mm(
    ...
    return _run_nongated_experts_grouped_mm_impl(w1, w2, _w3, x, num_tokens_per_expert, grouped_mm_quant="fp8")
...
def _run_nongated_experts_mxfp8_grouped_mm(
    ...
    return _run_nongated_experts_grouped_mm_impl(w1, w2, _w3, x, num_tokens_per_expert, grouped_mm_quant="mxfp8")
...
class LatentMoE(nn.Module):
    ...
    def __init__(
        ...
        self.experts = NonGatedGroupedExperts(
            ...
            grouped_mm_quant=grouped_mm_quant,
        )
        self.ep_comm_backend: EPCommBackend = "torch"
        self.experts.set_ep_comm_backend(self.ep_comm_backend)
