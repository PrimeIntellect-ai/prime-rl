...
- `routed_experts`: expose `layer.mlp._run_routed_experts(...)` for the MoE
  expert path, and optionally local expert helpers when local expert compute is
  separated from dispatch/combine.
...
def get_supported_targets(layer: nn.Module) -> frozenset[str]:
    ...
    if mlp is not None and (
        hasattr(mlp, "_run_routed_experts")
        or hasattr(mlp, "_run_local_routed_experts")
        or hasattr(mlp, "_run_sonic_routed_experts")
    ):
...
def set_selective_activation_checkpointing(layer: nn.Module, targets: Iterable[str]) -> None:
    ...
    if mlp is not None and "routed_experts" in enabled_targets:
        ...
        if hasattr(mlp, "_run_sonic_routed_experts"):
            checkpoint_method(mlp, "_run_sonic_routed_experts")
