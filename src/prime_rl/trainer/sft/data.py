...
class SFTDataset(StatefulIterableDataset):
    ...
    def _process_multimodal(self, example: dict) -> dict | None:
        ...
        # Prompt-only: determines loss mask boundary
        prompt_text = self.processor.apply_chat_template(
            prompt, tokenize=False, add_generation_prompt=True
        )
        prompt_inputs = self.processor(text=[prompt_text], images=images, return_tensors="pt", padding=False)
    ...
    def _has_images(self, example: dict) -> bool:
        ...
        for msg in self._parse_messages(example.get("prompt", [])) + self._parse_messages(example.get("completion", [])):
            content = msg.get("content")
