...
def train(config: RewardModelConfig):
    ...
    val_raw_dataset = load_preference_dataset(config.val.data) if config.val is not None else None
    ...
    def score_pairs(micro_batch: dict) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        pair_count = torch.tensor(num_pairs, dtype=torch.int64, device="cuda")
        return losses.sum(), pair_count, (margin > 0).sum(), margin.sum()

    def run_validation(step: int):
        assert config.val is not None and val_raw_dataset is not None
        val_dataset = setup_dataset(config.val.data, renderer, raw_dataset=val_raw_dataset, max_epochs=1)
