...
class MultiPacker(BasePacker):
    ...
    def _validate_sample(self, sample: TrainingSample) -> tuple[bool, str | None]:
        ...
        if sample.advantages is not None and len(sample.advantages) != sample_length:
            return (
                False,
                f"Run wrote a sample with advantages length != sample length ({len(sample.advantages)} != {sample_length})",
            )
        return True, None
