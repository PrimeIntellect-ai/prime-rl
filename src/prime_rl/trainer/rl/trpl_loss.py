...
def trpl_loss_fn(
    ...
    # PG loss: -advantages * importance_ratio
    pg_loss = (-advs * importance_ratio).mean()

    # Step 9: Projection loss KL(current || projected.detach()) on union + tail
    projected_lp_det = projected_lp.detach()
    projected_default_det = projected_default_lp.detach()
