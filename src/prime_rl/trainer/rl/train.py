...
def train(config: TrainerConfig):
    ...
    while True:
        ...
        should_broadcast_step_zero = progress.step == 0 and config.broadcast_at_step_zero
        if (progress.step > 0 or should_broadcast_step_zero) and (not last_async_level_steps or config.weight_broadcast.type == "filesystem"):
