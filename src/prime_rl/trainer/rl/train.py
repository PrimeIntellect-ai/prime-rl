...
from prime_rl.configs.trainer import DefaultLossConfig, SFTLossConfig, TrainerConfig
...
def train(config: TrainerConfig):
    ...
    while True:
        ...
        if (isinstance(config.loss, DefaultLossConfig) and config.loss.ratio_type == "token") or isinstance(
            config.loss, SFTLossConfig
        ):
