...
def _convert_hf_moe_to_tt(state_dict: dict[str, Tensor]):
    """Convert HF MoE format to TT-MoE format based on your script."""
    # Count MoE layers
    moe_layer_keys = [k for k in state_dict.keys() if "mlp.gate.weight" in k]
    num_layers = len(moe_layer_keys)

    if num_layers == 0:
        return  # No MoE layers

    # Count experts from first layer
