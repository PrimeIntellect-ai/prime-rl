...
the prime-rl trainer SLURM templates) PyTorch's caching allocator hands out
VMM-backed (``cuMemCreate`` + ``cuMemMap``) virtual ranges. ``ibv_reg_mr``
on such ranges succeeds, but the mlx5 HCA's MMU walk at WRITE time fails
with ``syndrome 0x4`` ("Local protection"), because ``nvidia_peermem``'s
``get_pages`` cannot pin a VA that spans multiple ``cuMemCreate`` handles.
UCX tears the endpoint down and NIXL surfaces it as ``REMOTE_DISCONNECT``.

Tensors handed to ``nixl_agent.register_memory`` must therefore come from a
classic, contiguous ``cudaMalloc`` block. This module exposes a ``MemPool``
backed by a ``CUDAPluggableAllocator`` calling ``cudaMalloc``/``cudaFree``
directly, plus :func:`classic_cuda_alloc` to scope specific allocations into
the pool. Everything else in the process keeps using the default caching
allocator.
"""
...
def classic_cuda_alloc() -> Iterator[None]:
    ...
    ``cudaMalloc`` block — currently only the NIXL-registered staging buffers.
    """
    pool = _get_pool()
