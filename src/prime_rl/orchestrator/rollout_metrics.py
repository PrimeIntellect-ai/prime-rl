...
def compute_rollout_metrics(
    ...
    *,
    ...
    # Solve rates (per group): a group is solved-none if no rollout earned reward, solved-all if
    # every configured slot did (sum == env group size). effective_batch_size is the remainder —
    # the groups with signal for GRPO.
    groups: dict = {}
    for r in rollouts:
