...
class Env:
    ...
    async def start(self) -> None:
        ...
        if self.config.is_legacy:
            # Bound the request: a server can accept connections yet never answer
            # (e.g. its pool worker died loading the env) — without a deadline the
            # run wedges at "Loading training environments" forever.
            try:
                info = await asyncio.wait_for(self.env_client.info(), timeout=ENV_SERVER_STARTUP_TIMEOUT)
            except TimeoutError:
                raise RuntimeError(
                    f"Env server {self.name} at {self.address} accepted connections but never served "
                    f"info within {ENV_SERVER_STARTUP_TIMEOUT}s — it likely failed loading the "
                    "environment; check the env server logs"
                )
