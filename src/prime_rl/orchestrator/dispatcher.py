...
- ``on_version_pending`` (called by the watcher before the engines pause for
  ...
  across weight updates and the ``TrainSink`` masks tokens past
  ``max_off_policy_steps`` out of the loss instead.
...
class RolloutDispatcher:
    ...
    async def on_version_pending(self, step: int) -> None:
        ...
        update and the ``TrainSink`` masks tokens past ``max_off_policy_steps``
        out of the loss. Eval rollouts are not aged because they are tied to
        their start-time policy version."""
        for meta in self.inflight.values():
            if meta.kind != "train":
                continue
