...
1. ``process_rollout`` — eager per-rollout tokenization (overlaps with
   ...
   rollouts skip this.
2. ``process_group`` — filters errored rollouts, masks tokens past
   ``max_off_policy_steps`` out of the loss, hands survivors to the env
...
def mask_stale_tokens(rollout: Rollout, min_version: int) -> tuple[int, int]:
    """Flip ``mask`` False on sampled tokens generated by a policy version older
    than ``min_version``, so they drop out of the loss (and its denominators)
    while staying in the sequence as context. Samples without per-token versions
    (the engine didn't report them) fall back to the rollout's start version —
    all-or-nothing per sample, the conservative direction. Returns
    ``(masked, sampled)`` token counts."""
    masked = 0
    sampled = 0
    for sample in rollout.samples:
        num_sampled = sum(sample.mask)
        sampled += num_sampled
        if not num_sampled:
            continue
        versions = sample.weight_versions
        if versions is None:
            if rollout.policy_version < min_version:
                sample.mask = [False] * len(sample.mask)
                masked += num_sampled
            continue
        mask = sample.mask
        for i, version in enumerate(versions):
            if mask[i] and version < min_version:
                mask[i] = False
                masked += 1
...
class TrainSink:
    ...
    def __init__(
        ...
        *,
        ...
        # Off-policy masking accounting; reset via ``reset_off_policy_stats``
        self.off_policy_sampled_tokens = 0
        self.off_policy_masked_tokens = 0
        self.off_policy_dropped_rollouts = 0
    ...
    async def process_group(self, group_id: uuid.UUID) -> None:
        ...
        when ``requires_group_scoring`` and any failed), mask off-policy
        tokens out of the loss, assign advantages, run pre-batch filters,
        ...
        # Token-level off-policy masking, before finalize_group so the
        # advantage broadcast and loss routing only land on fresh tokens.
        # Tokens sampled by a policy more than ``max_off_policy_steps``
        # versions behind the current one are flipped mask-False: they stay
        # in the sequence as context but contribute no loss. A rollout left
        # with no trainable token still shapes its group's advantages (its
        # reward stands), it just doesn't ship to the trainer.
        fully_stale: set[int] = set()  # id(rollout); Rollout __eq__ deep-compares traces
        num_masked = 0
        if env.sampler.samples_from_live_policy and self.config.max_off_policy_steps is not None:
            ...
            for r in survivors:
                masked, sampled = mask_stale_tokens(r, min_version)
                num_masked += masked
                self.off_policy_masked_tokens += masked
                self.off_policy_sampled_tokens += sampled
                if masked and not any(any(s.mask) for s in r.samples):
                    fully_stale.add(id(r))
            self.off_policy_dropped_rollouts += len(fully_stale)
        # Versions are consumed here; the trainer wire never carries them.
        for r in survivors:
            for sample in r.samples:
                sample.weight_versions = None
        ...
        get_logger().debug(
            ...
            f"fully_stale={len(fully_stale)}) | reward={avg_reward:.4f} | "
            f"masked_stale_tokens={num_masked} | filters: {filter_str}"
        )
