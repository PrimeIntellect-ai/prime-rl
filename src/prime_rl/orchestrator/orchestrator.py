...
async def orchestrate(config: OrchestratorConfig):
    ...
    while True:
        ...
        # Report inference usage (completions generated by vLLM)
        # Sum across all trajectory steps to capture multi-turn inference
        run_id = os.getenv("RUN_ID")
        if run_id:
            from prime_rl.utils.usage import get_usage_client

            total_inference_input = 0
            total_inference_output = 0
            for rollout in train_rollouts:
                inp, out = get_total_inference_tokens(rollout)
                total_inference_input += inp
                total_inference_output += out

            get_usage_client().report_usage(
                run_id=run_id,
                step=progress.step,
                tokens=total_inference_input + total_inference_output,
                input_tokens=total_inference_input,
                output_tokens=total_inference_output,
