...
class Orchestrator:
    ...
    def __init__(self, config: OrchestratorConfig) -> None:
        ...
        # Pulsed by ``on_new_version`` so a held ship can re-check the version
    ...
    async def finalize_train_batch(self, batch: TrainBatch) -> None:
        ...
        # Hold the ship until the trainer has published policy v{step-1-TARGET_LAG}.
    ...
    async def on_version_pending(self, step: int) -> None:
        ...
        still stepping) and buffered survivors in the sink. The cutoff is the
        oldest generation version that can still train within
        ``max_off_policy_steps``, were it to ship in the batch being collected
        (which trains on policy v{progress.step - 1})."""
