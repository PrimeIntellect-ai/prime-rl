...
# Maximum batches the orchestrator may run ahead of the trainer:
# ``hold_for_target_lag`` blocks shipping batch N until the trainer has
# published policy v{N-1-TARGET_LAG}. Dispatch needs no lead gate of its own —
# ``train_demand`` stops it once the batch being collected is covered, and the
...
class Orchestrator:
    ...
    def __init__(self, config: OrchestratorConfig) -> None:
        ...
        # Pulsed by ``on_new_version`` so ``hold_for_target_lag`` can re-check
    ...
    async def setup(self) -> None:
        ...
        self.dispatcher = RolloutDispatcher(
            ...
            train_demand=self.train_demand,
        )
    ...
    async def finalize_train_batch(self, batch: TrainBatch) -> None:
        ...
        await self.hold_for_target_lag(step)
        ...
        self.train_sink.drop_stale(self.stale_cutoff())
    ...
    def train_demand(self) -> int:
        """How many more train rollouts the dispatcher may start — injected as
        its scheduling bound, re-evaluated per scheduled group. Covers exactly
        the batch being collected (``progress.step``): buffered, partial-group,
        and in-flight rollouts all count toward coverage, so dispatch stops the
        moment the batch is covered and resumes when demand reappears (next
        batch, staleness drops, errored arrivals). Zero while rollouts started
        now would already exceed ``max_off_policy_steps`` in this batch."""
        ...
        return self.train_sink.remaining_rollout_demand(in_transit)

    def stale_cutoff(self) -> int:
        """Minimum generation policy version a rollout needs to train within
        ``max_off_policy_steps`` of it, were it to ship in the batch being
        collected: batch N trains on policy v{N-1}, so anything generated
        before v{N-1-max_off_policy_steps} is already unshippable."""
        return (self.progress.step - 1) - self.config.max_off_policy_steps

    async def hold_for_target_lag(self, step: int) -> None:
        """Block shipping batch ``step`` until the trainer has published policy
        v{step - 1 - TARGET_LAG}. Demand only stops *new* rollouts; on tiny fast
        envs already-covered batches complete faster than the trainer consumes
        them, letting the orchestrator finish all its steps and tear down while
        the trainer still has pending NCCL broadcasts — which then hang forever
        without the watcher to serve the receive handshake. Holding here keeps
        the batch counter within TARGET_LAG of the trainer instead.

        Never waits for an unpublishable version: under NCCL the trainer's last
        broadcast is v{max_steps - 2} (``nccl_broadcast_unused``), exactly what
        the final batch requires. Bench runs have no trainer publishing
        versions, so they ship freely."""
    ...
    async def on_version_pending(self, step: int) -> None:
        ...
        weight update: drop everything already past the staleness cutoff —
        in-flight groups (their aborts must be processed while the engine is
        still stepping) and buffered survivors in the sink."""
        cutoff = self.stale_cutoff()
    ...
    async def on_new_version(self, step: int) -> None:
        ...
        wake a ship held in ``hold_for_target_lag``."""
