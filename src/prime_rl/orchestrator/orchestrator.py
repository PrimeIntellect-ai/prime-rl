...
async def orchestrate(config: OrchestratorConfig):
    ...
    while True:
        ...
        input_tokens = int(results_df.prompt_len.sum())
        output_tokens = int(results_df.completion_len.sum())
        ...
        # Report inference usage for billing (completions generated by vLLM)
        run_id = os.getenv("RUN_ID")
        if run_id:
            from prime_rl.utils.billing import get_billing_client

            get_billing_client().report_usage(
                run_id=run_id,
                step=progress.step,
                tokens=num_tokens,
