...
the config, ``wire()`` binds their hooks, and ``start()`` runs their tasks until
the shipper drains the pipeline. The components:

- ``Dispatcher`` schedules environment runs and delivers each episode once.
- ``TrainSink`` scores finished train groups and compiles them into samples.
- ``Queue`` buffers finished groups, voids stale ones, and cuts batches.
...
bound here, so each one constructs and replays standalone in a test. Everything
that needs closing registers on one ``AsyncExitStack`` as it comes up, so a setup
that fails halfway tears down exactly what it built.
"""
...
class Orchestrator:
    # Built by ``setup()``, which registers each one's teardown on ``closers``.
    ...
    inference_metrics: InferenceMetricsCollector
    sink: TrainSink
    queue: Queue
    ...
    async def setup(self) -> None:
        ...
        self.closers.push_async_callback(clients.aclose)
        admin_plane = setup_admin_plane(config.model.client, config.model.name)
        self.closers.push_async_callback(admin_plane.aclose)
        ...
        self.train_envs = TrainEnvs(
            config.train.source, config.env_addresses, config_dir, clients=clients, renderer_config=config.renderer
        )
        self.eval_envs = (
        ...
        if config.resume is not None:
            if config.resume.dir is not None:
                ...
                if resume_step is None:
                    resume_step = resolve_latest_ckpt_step(get_ckpt_dir(config.output_dir))
        get_logger().info(f"Resuming from step {resume_step}" if resume_step is not None else "Starting from scratch")
        ...
        self.shipper = Shipper(config, train_source=train_source, resume_step=resume_step)
        self.closers.callback(self.shipper.close)

        get_logger().info("Waiting for policy inference pool to be ready")
        t0 = time.perf_counter()
        await admin_plane.wait_for_ready(config.model.name)
        ...
        self.closers.push_async_callback(self.close_frozen_clients)
        ...
        self.closers.push_async_callback(self.watcher.stop)
        ...
        if config.eval is not None and self.eval_envs is not None:
            eval_source = EvalSource(
                ...
                is_resumed=resume_step is not None,
            )
        ...
        self.closers.push_async_callback(self.dispatcher.stop)
        self.inference_metrics = InferenceMetricsCollector(
            admin_plane.clients, roles=config.inference_metrics_roles, log=config.collect_inference_metrics
        )
        self.closers.push_async_callback(self.inference_metrics.stop)
        self.sink = TrainSink(self.train_envs)
        self.queue = Queue(batch_size=config.batch_size, max_off_policy_steps=config.max_off_policy_steps)
        self.periodic_logger = PeriodicLogger(name="Pipeline", interval=config.log.interval)
        self.closers.push_async_callback(self.periodic_logger.stop)
        ...
        sync_version = resume_step if resume_step is not None else 0
    ...
    async def close_frozen_clients(self) -> None:
    ...
    def wire(self, train_source: TrainSource) -> None:
        ...
        dispatcher, watcher, shipper, evaluator = self.dispatcher, self.watcher, self.shipper, self.evaluator
        ...
        dispatcher.bind(
            ...
            on_eval=evaluator.ingest if evaluator is not None else None,
        ...
        if evaluator is not None:
            ...
            on_new_version.append(evaluator.trigger)
        ...
        if evaluator is not None:
            logger.register(status=evaluator.status)
    ...
    async def start(self) -> None:
        ...
        lag_task = asyncio.create_task(self.lag_monitor.run(), name="event_loop_lag")
        self.closers.push_async_callback(lambda: safe_cancel(lag_task))
    ...
    async def stop(self) -> None:
        """Close everything ``setup()`` built, newest first, within a global timeout so a
        ...
        closing = asyncio.create_task(self.closers.aclose())
        _, pending = await asyncio.wait({closing}, timeout=SHUTDOWN_TIMEOUT_S)
        if pending:
            get_logger().warning(
