...
class Orchestrator:
    ...
    async def finalize_train_batch(self, batch: TrainBatch) -> None:
        ...
        if self.train_sink.off_policy_sampled_tokens > 0:
            metrics["off_policy/masked_token_rate"] = (
                self.train_sink.off_policy_masked_tokens / self.train_sink.off_policy_sampled_tokens
            )
            metrics["off_policy/fully_stale_rollouts"] = self.train_sink.off_policy_dropped_rollouts
