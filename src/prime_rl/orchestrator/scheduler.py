    ...
    async def update_policy(self):
        ...
        if next_ckpt_step > self.ckpt_step:
            ...
            if len(tasks_to_remove) > 0:
                self.logger.warning(
                    f"Cancelled {len(tasks_to_remove)} group rollout requests because they would be generated by >{self.max_off_policy_steps} policy versions. These requests will refill naturally. To avoid cancellation, consider increasing max_off_policy_steps."
                )
