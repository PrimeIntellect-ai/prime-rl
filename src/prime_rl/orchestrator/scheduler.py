...
class Scheduler:
    ...
    async def maybe_update_policy(self):
        ...
        if next_ckpt_step > self.ckpt_step:
            ...
            if cancelled > 0:
                self.logger.warning(
                    f"Cancelled {cancelled} group rollout requests because they would be generated by >{self.max_off_policy_steps} policy versions. These requests will refill naturally. To avoid cancellation, consider increasing max_off_policy_steps."
                )
