...
def monkey_patch_session_id_header():
    ...
    at the inference router level for KV cache affinity without affecting
    client caching.

    Patches OpenAIChatCompletionsClient.get_native_response and
    OpenAIChatCompletionsTokenClient.get_native_response to extract
    example_id from the state kwarg and pass it as extra_headers to
    create() / client.post().

    Applied in both the orchestrator process (via orchestrator.py) and the
    env server subprocess (via vf_utils._run_env_server_with_patches).
    """
    ...
    # --- Patch 1: OpenAIChatCompletionsClient ---
    @handle_openai_overlong_prompt
    ...
    # --- Patch 2: OpenAIChatCompletionsTokenClient ---
