...
class OPD(vf.Algorithm[AlgorithmConfig]):
    ...
    async def advantage(self, traces: list[vf.Trace]) -> list[vf.Trace]:
        ...
        branch_refs: list[tuple[vf.Trace, vf.Branch]] = []
        for trace in traces:
            for branch in trace.branches:
                ...
                branch_refs.append((trace, branch))

        scored = await asyncio.gather(*calls) if calls else []
        for (trace, branch), reference_logprobs in zip(branch_refs, scored, strict=True):
            sampled_mask = [sampled and not trace.has_error for sampled in branch.sampled_mask]
            branch.advantages = [
...
class OPSD(vf.Algorithm[AlgorithmConfig]):
    ...
    async def advantage(self, traces: list[vf.Trace]) -> list[vf.Trace]:
        ...
        calls: list[asyncio.Task[list[float]]] = []
        branch_refs: list[tuple[vf.Trace, vf.Branch, int]] = []
        for trace in traces:
            ...
            branch_refs.append((trace, branch, len(sampled_token_ids)))

        scored = await asyncio.gather(*calls) if calls else []
        for (trace, branch, sampled_count), full_reference_logprobs in zip(branch_refs, scored, strict=True):
