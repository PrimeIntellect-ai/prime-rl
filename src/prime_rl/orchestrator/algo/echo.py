...
class EchoAlgorithm(GRPOAlgorithm):
    ...
    async def score_trace(
        ...
        self._weight_observations(trace, prepared)

    def _weight_observations(self, trace: vf.Trace, prepared: PreparedTrace) -> None:
        ...
        # Same branch selection as ``trace_to_samples``, so the zip with ``prepared``
        ...
        for sample_idx, (sample, branch) in enumerate(zip(prepared, trainable_branches)):
