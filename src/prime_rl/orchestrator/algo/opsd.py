...
class OPSDAlgorithm(Algorithm):
    ...
    def __init__(self, config: OPSDAlgoConfig, policy_pool: InferencePool):
        ...
        self._semaphore: asyncio.Semaphore | None = None  # bounds concurrent policy queries; created in setup()
    ...
    async def score_rollout(self, rollout: Rollout) -> None:
        ...
        assert renderer is not None and semaphore is not None, "Algorithm.setup() must run first"
