...
- ``algorithm`` — :class:`Algorithm`, one strategy object per env and the only
  orchestrator component that interprets ``AlgorithmConfig``; owns the pools
  for inline frozen model references.
...
from prime_rl.orchestrator.algo.algorithm import Algorithm, score_train_batch, setup_frozen_pool
