...
``{prefix}/{subset}/<metric>/<stat>``. Both layer on ``common_metrics`` for the metrics shared
...
def dist_stats(prefix: str, values: list[float]) -> dict[str, float]:
    """mean/max/min of a value distribution keyed under ``prefix``; ``{}`` for an empty list."""
    if not values:
        return {}
    return {
...
def rate(flags: list[bool]) -> float:
    """Fraction of ``flags`` that are True (0.0 for an empty list)."""
...
def common_metrics(
    ...
    out |= dist_stats(f"{p}/num_total_tokens", [float(r.num_total_tokens) for r in rollouts])
    out |= dist_stats(f"{p}/num_input_tokens", [float(r.num_input_tokens) for r in rollouts])
    out |= dist_stats(f"{p}/num_output_tokens", [float(r.num_output_tokens) for r in rollouts])
    out |= dist_stats(f"{p}/num_turns", [float(r.num_turns) for r in rollouts])
    out |= dist_stats(f"{p}/num_branches", [float(r.num_branches) for r in rollouts])
    ...
    out |= dist_stats(f"{p}/timing/setup", setup)
    out |= dist_stats(f"{p}/timing/generation", generation)
    out |= dist_stats(f"{p}/timing/finalize", finalize)
    out |= dist_stats(f"{p}/timing/scoring", scoring)
    out |= dist_stats(f"{p}/timing/total", [sum(spans) for spans in zip(setup, generation, finalize, scoring)])
    ...
    for name in sorted({name for r in rollouts for name in r.metrics}):
        out |= dist_stats(f"{p}/metrics/{name}", [r.metrics[name] for r in rollouts if name in r.metrics])
    for name in sorted({name for r in rollouts for name in r.rewards}):
        out |= dist_stats(f"{p}/rewards/{name}", [r.rewards[name] for r in rollouts if name in r.rewards])

    # Per-rollout boolean rates; has_error is structurally 0 on `effective`, so log it on `all` only
    out[f"{p}/is_truncated/mean"] = rate([r.is_truncated for r in rollouts])
    out[f"{p}/is_completed/mean"] = rate([r.is_completed for r in rollouts])
    if subset == "all":
        out[f"{p}/has_error/mean"] = rate([r.has_error for r in rollouts])
    ...
    out[f"{p}/stop_condition/generation_truncated"] = rate(
...
def compute_train_metrics(
    ...
    out = common_metrics(rollouts, prefix=prefix, subset=subset, env_group_size=env_group_size)
    if not rollouts:
        return out
    p = f"{prefix}/{subset}"
    out |= dist_stats(f"{p}/reward", [float(r.reward) for r in rollouts])
    out[f"{p}/is_filtered/mean"] = rate([r.is_filtered for r in rollouts])
    for name in sorted({name for r in rollouts for name in r.filter_results}):
        out[f"{p}/filters/{name}/mean"] = rate([r.filter_results.get(name, False) for r in rollouts])
    return out
