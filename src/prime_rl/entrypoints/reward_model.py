...
REWARD_MODEL_TOML = "reward_model.toml"
REWARD_MODEL_SBATCH = "reward_model.sbatch"


def write_config(config: RewardModelConfig, config_path: Path, exclude: set[str] | None = None) -> None:
...
def write_slurm_script(config: RewardModelConfig, config_path: Path, script_path: Path) -> None:
...
def reward_model_slurm(config: RewardModelConfig):
    """Run reward-model training via SLURM."""
    assert config.slurm is not None

    logger = setup_logger(config.log.level or "info", json_logging=config.log.json_logging)

    config_dir = get_config_dir(config.output_dir)
    config_path = config_dir / REWARD_MODEL_TOML
    exclude = (
    ...
    script_path = config.output_dir / REWARD_MODEL_SBATCH
...
def reward_model_local(config: RewardModelConfig):
    """Run reward-model training locally with process monitoring and cleanup."""
    ...
    if config.dry_run:
        logger.success("Dry run complete. To start a reward-model run locally, remove --dry-run from your command.")
        return
    ...
    trainer_cmd = [
        ...
        "-m",
        "prime_rl.trainer.reward_model.train",
        "@",
        (config_dir / REWARD_MODEL_TOML).as_posix(),
    ]

    logger.info(f"Starting reward-model trainer with {config.deployment.num_gpus} GPU(s)")
...
def reward_model(config: RewardModelConfig):
    ...
    if config.slurm is not None:
        reward_model_slurm(config)
    else:
        reward_model_local(config)
