...
@dataclass
class OrchState:
    """Minimal orch2 state needed to resume cleanly. Engine version + watcher
    cursor are recoverable from disk on the next watcher tick, so we don't
    persist them. Round-robin scheduler index resets to 0 on resume — minor
...
class CkptManager:
    ...
    `<output_dir>/checkpoints/step_N/orchestrator/state.pt` — matching orch1's
    layout so trainer + orch ckpts at the same step share a parent dir."""
